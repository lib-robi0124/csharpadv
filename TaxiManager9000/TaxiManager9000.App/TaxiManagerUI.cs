            ...
            while (true)
            {
                ...
                switch (chosenMenuOption)
                {
                    ...
                    case MenuChoice.ListAllDrivers:
                        _driverService.GetAll().Print();

                        break;
                    case MenuChoice.TaxiLicenseStatus:
                        _driverService.GetAll().PrintStatus();
                        break;
                        ...
                        List<DriverManagerChoise> driverMangerMenu = new List<DriverManagerChoise>()
                {
                    DriverManagerChoise.AssigneDriver, DriverManagerChoise.UnassigneDriver
                        ...
                        int driverChoice = _uiService.ChooseMenu(driverMangerMenu);

                        List<Driver> availableDriver = _driverService.GetFiltered(x => _driverService.IsAvailableDriver(x));

                        if (driverChoice == 1)
                        {
                            List<Driver> availableForAssigneDrivers = availableDriver.Where(x => x.Car == null).ToList();

                            int assigneDriverChoise = _uiService.ChooseEntitiesMenu(availableForAssigneDrivers);

                            if (assigneDriverChoise == -1) continue;

                            List<Car> availableForAssigneCars = _carService.GetFiltered(x => _carService.IsAvailableCar(x));

                            int assigneCarChoise = _uiService.ChooseEntitiesMenu(availableForAssigneCars);
                            if (assigneCarChoise == -1) continue;

                            _driverService.AssignDriver(availableForAssigneDrivers[assigneDriverChoise - 1], availableForAssigneCars[assigneCarChoise - 1]);
                        }
                        else if (driverChoice == 2)
                        {
                            List<Driver> availableForUnAssigneDrivers = availableDriver.Where(x => x.Car != null).ToList();

                            int unAssigneDriverChoise = _uiService.ChooseEntitiesMenu(availableForUnAssigneDrivers);
                            if (unAssigneDriverChoise == -1) continue;

                            _driverService.UnassignDriver(availableForUnAssigneDrivers[unAssigneDriverChoise - 1]);
                        }
                        break;
